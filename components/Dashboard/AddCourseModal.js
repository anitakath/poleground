...
const AddCourseModal = ({ onClose, courseData}) => {
  ...
  // mehrere Daten statt nur ein Feld
  const [dates, setDates] = useState([{ scheduled_at: "", duration: "" }]);
  ...
  const addDateField = () => {
    setDates([...dates, { scheduled_at: "", duration: "" }]);
  };
  ...
  const submitHandler = async (e) => {
    ...
    // VALIDIERUNG!
    const validationErrors = validateForm(formData, courseData, dates);
    ...
    // mehrere Kursobjekte aus den Dates generieren
    const coursesToInsert = dates.map((date) => {
      const newUuid = uuidv4();

      let spotsValue = "10/10";
      ...
      return {
        ...
        uuid: newUuid,
        spots: spotsValue,
        scheduled_at: date.scheduled_at ? date.scheduled_at + ":00" : null,
    ...
    try {
      ...
      if (error) {
        ...
        setMessages({ error: null, success: 'Alle Kurse wurden erfolgreich gespeichert!' });
      }
    } catch (err) {
      ...
      <div className={styles.modalContent}>
        ...
        <form className={styles.form} onSubmit={submitHandler}>
          ...
          <div className={styles.schedulesContainer}>
            {dates.map((date, index) => (
              <div key={index} className="flex gap-4 mb-2">
                <div className={styles.selectContainer}>
                  ...
                  <input
                    type="datetime-local"
                    value={date.scheduled_at}
                    onChange={(e) => handleDateChange(index, "scheduled_at", e.target.value)}
                    required
                  />
                </div>
                <div className={styles.selectContainer}>
            ...
            <button type="button" className={styles.addButton} onClick={addDateField}>
